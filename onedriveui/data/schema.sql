-- OneDriveUI — FROZEN CONTRACT: the complete SQLite schema.
--
-- Transcribed from ARCHITECTURE.md §10. Owned by WP-00; every other work package
-- reads it and none may edit it. Applied by data/db.py to a fresh database; an
-- existing database is brought forward by migrations/NNN_*.sql instead.
--
-- Two rules the DDL below encodes and every writer must honour:
--   * Every table has exactly ONE writing module (named in the comment above it).
--     Everyone else reads. This is what keeps parallel packages from colliding.
--   * Every per-file row is keyed by rel_path — POSIX, relative to the account's
--     sync root. NEVER an inode: rclone inodes are not stable across remounts,
--     and both xattrs and gio metadata:: fail on the rclone FUSE mount.
--
-- The database is a derived index and a history. It is never the only copy of
-- user data, which is why db.py may rename a corrupt file aside and start again.
